...
#define THREADS 20
#define EXP_LOOKUP_RES 1000
#define DERIV_THRESHOLD 1e-10
 
double min_exponent = -7;
FILE *output;
FILE *lor_file;
double exp_lookup[EXP_LOOKUP_RES];
graph sensitivity;
...
void *worker(void *arg) {
  ...
  // Preallocate buffers once per thread — no malloc/free inside LOR loop
  int max_bbox = X_RES * Y_RES * Z_RES;
  double *derivs  = (double *)malloc(max_bbox * sizeof(double));
  int    *indices = (int    *)malloc(max_bbox * sizeof(int));
  ...
  for (int r = 0; r < data->num_lors; r++) {
    ...
    // Incremental coordinate computation — avoids multiply per voxel
    double x = x_const + current_lor->i_left * x_increment;
    for (int i = current_lor->i_left; i <= current_lor->i_right; i++, x += x_increment) {
      double y = y_const + current_lor->j_left * y_increment;
      for (int j = current_lor->j_left; j <= current_lor->j_right; j++, y += y_increment) {
        double z = z_const + current_lor->k_left * z_increment;
        for (int k = current_lor->k_left; k <= current_lor->k_right; k++, z += z_increment) {
          int h = i * Y_RES * Z_RES + j * Z_RES + k;
          vec3d displacement = vec_sub(three_vec(x, y, z), current_lor->center);
          double exponent =
              -0.5 * vec_dot(displacement, sym_transform(&current_lor->new_dot,
          ...
          // Skip negligible derivatives to save accumulation work
          if (derivative < DERIV_THRESHOLD) continue;
 
          likelihood += derivative * data->current_image->values[h];
          derivs[count]  = derivative;
          indices[count] = h;
          count++;
        }
      }
    ...
    if (likelihood > 1e-9) {
      ...
      for (int s = 0; s < count; s++)
        result->values[indices[s]] += derivs[s] * one_over_likelihood;
      one_over_likelihood_sum += one_over_likelihood;
    }
  }
...
int main(int argc, char **argv) {
  ...
  // Allocate large structs on heap
  grid  *cells       = (grid  *)calloc(1, sizeof(grid));
  graph *voxels      = (graph *)calloc(1, sizeof(graph));
  graph *prev_voxels = (graph *)calloc(1, sizeof(graph));
  ...
  printf("Total LORs: %u | In-bounds: %u\n", num_lors, num_comps);
  printf("Initializing threads...\n");
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    graph *thread_results[THREADS] = {0};
    graph *complete_expectation = (graph *)calloc(1, sizeof(graph));
    double total_background_update_sum = 0;
    ...
    iteration++;
    printf("Iteration %i/%i | Change (MAE): %e | Background: %e\n",
           iteration, iterations, change, voxels->background);
